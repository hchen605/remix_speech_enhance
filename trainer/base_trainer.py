...
class BaseTrainer:
    def __init__(self, config, resume: bool, model, optimizer, scheduler):
        ...
        with open((self.root_dir / f"config.json").as_posix(), "w") as handle:
    ...
    def _print_networks(self, nets: list):
        ...
        for i, net in enumerate(nets, start=1):
            ...
            self.writer.add_text(
                tag=f"\tNetwork {i}",
                text_string=f" \tNetwork {i} contains {params_of_network / 1e6} million parameters.",
                global_step=1
            )
    ...
    def metrics_visualization(self, noisy_list, clean_list, enhanced_list, epoch):
        ...
        for i, (noisy, enhanced, clean) in enumerate(zip(noisy_list, enhanced_list, clean_list)):
            try:
        ...
        print('SI-SDR_clean_denoise: ',np.mean(sisdr_clean_denoise))
